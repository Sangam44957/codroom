/**
 * Testable socket app factory. Accepts injected roomState + services so tests
 * can pass in-memory fakes without Redis or Postgres.
 */
...
export function createSocketApp({ roomState, services, jwtSecret }) {
  ...
  function isAllowed(socketId, event) {
    ...
    const now = Date.now();
    if (!buckets[event]) buckets[event] = { tokens: limits.capacity, lastRefill: now };
  ...
  async function seedRoomFromDB(roomId) {
    try {
      ...
      return {
        ...
        lastCode: data?.interview?.snapshots?.[0]?.code || "",
        interviewId: data?.interview?.status === "in_progress" ? data.interview.id : null,
  ...
  async function updateRoomOnCandidateJoin(roomId, candidateName) {
    ...
    return { wasFirstJoin: room.status === "waiting" };
  }
  ...
  io.use(async (socket, next) => {
    try {
      ...
      if (!token) { socket.data.user = null; socket.data.isAuthenticated = false; return next(); }
  ...
  function boundRoom(socket, payloadRoomId) {
    ...
    return bound && bound === payloadRoomId ? bound : null;
  }
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join-room", async ({ roomId, userName }) => {
      ...
      if (socket.data.isAuthenticated && socket.data.user) {
        ...
        if (data.createdById === socket.data.user.userId) {
          ...
          } catch { socket.emit("join-error", { message: "Invalid room ticket" }); return; }
        }
      } else {
        const ticket = socket.handshake.auth?.roomTicket;
      ...
      } catch { socket.emit("join-error", { message: "Room initialization failed" }); return; }
    ...
    socket.on("disconnect", async () => {
      ...
      if (users.length === 0) {
        const key = `${roomId}-snapshot`;
        if (snapshotTimers.has(key)) { clearTimeout(snapshotTimers.get(key)); snapshotTimers.delete(key); }
