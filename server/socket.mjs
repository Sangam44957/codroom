...
const httpServer = createServer(async (req, res) => {
  ...
  // Code execution endpoint — proxied from Vercel
  if (req.method === "POST" && req.url === "/execute") {
    ...
    req.on("end", async () => {
      try {
        ...
        const { submitCode, parseResult } = await import("./executor.mjs");
        const rawResult = await submitCode(code, language, stdin || "");
        const result = parseResult(rawResult);

        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify(result));
      } catch (err) {
        logger.error({ err: err.message }, "Code execution failed");
        res.writeHead(500, { "Content-Type": "application/json" });
        res.end(JSON.stringify({
          error: "DOCKER_UNAVAILABLE",
          stderr: err.message || "Execution sandbox unavailable",
        }));
      }
    });
    return;
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", async ({ roomId, userName, role: clientRole }) => {
    ...
    if (socket.data.isAuthenticated && socket.data.user) {
      ...
      try {
        ...
        if (payload.sessionId && pubClient?.isReady) {
          const sessKey = `sess:${payload.sessionId}`;
          const existing = await pubClient.get(sessKey);
          if (existing && existing !== socket.id) {
            socket.emit("join-error", { message: "Session already active" });
            return;
          }
          await pubClient.set(sessKey, socket.id, { EX: 14400, NX: false });
          socket.data.sessionKey = sessKey;
        }
      } catch(err) {
    ...
    let existingUsers = [];
    if (roomState) {
      const rawUsers = await roomState.getRoomUsers(roomId);
      existingUsers = Array.isArray(rawUsers) ? rawUsers : [];
    }
    const duplicateConnections = existingUsers.filter(u => 
      u && u.name === userName && u.role === authoritativeRole && u.id !== socket.id
    );
    ...
    try {
      ...
      if (!room) {
        ...
        // initRoom is atomic — whether we wrote or lost the race, state is now valid
        room = await roomState.getRoomState(roomId);
      }
    } catch (error) {
  ...
  socket.on("set-collab-mode", async ({ roomId: rid, enabled }, ack) => {
    const roomId = boundRoom(socket, rid);
    if (!roomId || socket.data.role !== "interviewer") {
      ack?.({ ok: false, error: "Unauthorized" });
      return;
    }
    io.to(roomId).emit("collab-mode-changed", { enabled: !!enabled });
    slog.info({ roomId, enabled: !!enabled }, "collab mode changed");
    ack?.({ ok: true });
  });
  ...
  socket.on("cursor-move", async ({ roomId: rid, cursor }) => {
    const roomId = boundRoom(socket, rid);
    if (!roomId) return;
    if (!await isAllowed(socket.id, "cursor-move")) return;
  ...
  socket.on("emoji-reaction", ({ roomId: rid, emoji }) => {
    const roomId = boundRoom(socket, rid);
    if (!roomId) return;
    const allowed = ["👍", "😕", "⏳", "🎉"];
  ...
  socket.on("disconnect", async () => {
    ...
    if (socket.data.sessionKey && pubClient?.isReady) {
      pubClient.del(socket.data.sessionKey).catch(() => {});
    }
