...
function StatCard({ icon: Icon, label, value, sub, color = "violet" }) {
  const accentColor = ACCENT_COLORS[color] ?? ACCENT_COLORS.violet;
  return (
    <motion.div
    ...
    >
      <div
        className="glow-orb -bottom-6 -left-6 h-24 w-24"
        style={{ background: accentColor }}
      />
      <div className="relative flex items-center justify-between mb-3">
        <p
          className="text-[var(--text-caption)] uppercase tracking-wide text-slate-400"
      ...
      <p
        className="relative text-[var(--text-headline)] text-white"
        style={{ fontWeight: "var(--weight-headline)" }}
      >
        {value ?? "—"}
      </p>
      {sub && (
        <p className="relative text-[var(--text-caption)] text-slate-600 mt-1">{sub}</p>
      )}
    </motion.div>
  );
}
        ...
        <main className="max-w-7xl mx-auto px-6 py-10">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-black text-white tracking-tight font-[family-name:var(--font-display)]">Analytics</h1>
