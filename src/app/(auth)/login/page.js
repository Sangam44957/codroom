...
function AuroraOrbs() {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
      <div className="fixed inset-0 z-0" style={{ background: "radial-gradient(ellipse 120% 80% at 20% -10%, #1e1040 0%, #0d0d1a 45%, #080c18 100%)" }} />
      <motion.div
        animate={{ x: [0, 40, -20, 0], y: [0, -30, 20, 0] }}
        transition={{ duration: 18, repeat: Infinity, ease: "easeInOut" }}
        style={{ position: "absolute", width: 700, height: 700, top: -250, left: -150, background: "radial-gradient(circle, rgba(120,80,255,0.22) 0%, transparent 65%)", filter: "blur(130px)" }}
      />
      <motion.div
        animate={{ x: [0, -50, 30, 0], y: [0, 40, -20, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut", delay: 3 }}
        style={{ position: "absolute", width: 500, height: 500, top: "20%", right: -150, background: "radial-gradient(circle, rgba(56,189,248,0.14) 0%, transparent 65%)", filter: "blur(110px)" }}
      />
      <motion.div
        animate={{ x: [0, 30, -40, 0], y: [0, -20, 30, 0] }}
        transition={{ duration: 26, repeat: Infinity, ease: "easeInOut", delay: 6 }}
        style={{ position: "absolute", width: 400, height: 400, bottom: 0, left: "35%", background: "radial-gradient(circle, rgba(244,114,182,0.12) 0%, transparent 65%)", filter: "blur(120px)" }}
      />
    </div>
  );
}
...
function Field({ label, type, placeholder, value, onChange, error, right }) {
  return (
    <div>
      <label className="block text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: "rgba(255,255,255,0.4)", letterSpacing: "0.1em" }}>
        {label}
      </label>
      ...
      >
        ...
        <ShimmerBorder>
          <div className="rounded-3xl px-8 py-9">
            ...
            {successMsg && (
              <div className="mb-5 p-3 rounded-xl text-sm" style={{ background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)", color: "#6ee7b7" }}>
                {successMsg}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs font-semibold tracking-widest uppercase" style={{ color: "rgba(255,255,255,0.4)", letterSpacing: "0.1em" }}>Password</label>
                  <Link
...
export default function LoginPage() {
  return (
    <Suspense fallback={<div className="min-h-screen" style={{ background: "#080c18" }} />}>
      <LoginInner />
    </Suspense>
  );
}
