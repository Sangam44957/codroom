...
export default function PlaybackPage() {
  ...
  const [annotations, setAnnotations] = useState([]);   // { id, timelineIndex, timestamp, text }
  ...
  // Persist annotations in localStorage per interview
  const annotationKey = data?.interview?.id ? `codroom:annotations:${data.interview.id}` : null;

  useEffect(() => {
    if (!annotationKey) return;
    try {
      const saved = localStorage.getItem(annotationKey);
  ...
  function saveAnnotation() {
    ...
    if (!text || !annotationKey) return;
    const entry = {
      id: Date.now(),
      timelineIndex: currentIndex,
      timestamp: timeline[currentIndex]?.timestamp ?? new Date().toISOString(),
      text,
    };
    const next = [...annotations, entry].sort((a, b) => a.timelineIndex - b.timelineIndex);
    setAnnotations(next);
    localStorage.setItem(annotationKey, JSON.stringify(next));
    setAnnotationDraft("");
  ...
  function deleteAnnotation(id) {
    ...
    setAnnotations(next);
    if (annotationKey) localStorage.setItem(annotationKey, JSON.stringify(next));
  }
  ...
  useEffect(() => {
    if (showAnnotationInput) setTimeout(() => annotationInputRef.current?.focus(), 50);
  }, [showAnnotationInput]);
  ...
  return (
    <div className="h-screen flex flex-col bg-[#080810] overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        {showAnnotationInput && (
          <div className="absolute bottom-[120px] left-1/2 -translate-x-1/2 z-20 w-full max-w-sm px-4">
            <div className="bg-[#0f0f1a] border border-amber-500/30 rounded-xl shadow-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-3 py-2 border-b border-white/[0.06]">
                <Bookmark size={12} className="text-amber-400" />
                <span className="text-xs text-amber-300 font-semibold flex-1">
                  Annotation @ {timeline[currentIndex] ? formatTime(timeline[currentIndex].timestamp) : "--"}
                </span>
                <button onClick={() => setShowAnnotationInput(false)} className="text-slate-500 hover:text-white">
                  <X size={12} />
                </button>
              </div>
              <textarea
                ...
                placeholder="Add a note about this moment… (Enter to save)"
                rows={2}
                className="w-full px-3 py-2 bg-transparent text-xs text-slate-300 placeholder-slate-600 focus:outline-none resize-none"
              />
              <div className="flex justify-end gap-2 px-3 py-2 border-t border-white/[0.06]">
                <button
                ...
                <button
                  ...
                  className="px-3 py-1 text-xs bg-amber-600 hover:bg-amber-500 text-white rounded-lg font-semibold transition-all disabled:opacity-40"
                >Save</button>
              </div>
            </div>
          </div>
        ...
        {showEvents && sidebarItems.length > 0 && (
          <div className="w-56 flex-shrink-0 border-l border-white/[0.05] bg-[#0a0a12] flex flex-col">
            ...
            <div ref={eventListRef} className="flex-1 overflow-y-auto py-1">
              {/* Annotations mixed into event list */}
              {annotations.map((ann) => (
                <div
                  key={ann.id}
                  className="group flex items-start gap-2.5 px-3 py-2 border-b border-white/[0.03] hover:bg-white/[0.03] transition-colors"
                >
                  <span className="mt-1 w-2 h-2 rounded-full flex-shrink-0 bg-amber-400" />
                  <div className="min-w-0 flex-1">
                    <div className="text-xs font-medium text-amber-300 truncate">📝 {ann.text}</div>
                    <div className="text-[10px] text-slate-600 font-mono mt-0.5">{formatTime(ann.timestamp)}</div>
                  </div>
                  <button
