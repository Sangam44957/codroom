...
import SkillGapPanel from "@/components/ui/SkillGapPanel";
...
export default function ReportPage() {
  ...
  async function handleShareModal() {
    const interviewId = room?.interview?.id;
    if (!interviewId) return;
    setSharing(true);
    try {
  ...
  const animatedScore = useCountUp(report ? clamp(report.overallScore, 0, 100) : 0);
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-6 py-24 text-center">
          {polling ? (
            ...
            <>
              ...
              <h1 className="text-3xl font-black text-white mb-3 font-[family-name:var(--font-display)]">Generate AI Report</h1>
    ...
    <>
      {showShareModal && (
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center px-4">
          <div className="w-full max-w-md bg-[#0f0f14] border border-white/[0.08] rounded-2xl p-8 text-center shadow-2xl">
            <div className={`inline-flex items-center gap-3 px-5 py-3 rounded-xl border ${rec.bg} mb-6`}>
              <RecIcon size={22} className={rec.color} />
              <span className={`text-xl font-black ${rec.color}`}>{rec.label}</span>
              <span className="text-slate-500 text-sm">· {clamp(report.overallScore, 0, 100)}/100</span>
            </div>
            <h2 className="text-white font-black text-2xl mb-2">Report ready</h2>
            <p className="text-slate-500 text-sm mb-8 leading-relaxed">
              Share this report with your hiring committee so they can review the AI evaluation.
            </p>
            <button
              onClick={handleShareModal}
              disabled={sharing}
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white rounded-xl font-bold text-sm transition-all disabled:opacity-50 mb-3"
            >
              {copied ? <><Check size={16} /> Link copied to clipboard!</> : sharing ? "Generating link…" : <><Share2 size={16} /> Share with hiring committee</>}
            </button>
            {shareUrl && (
              <div className="flex items-center gap-2 p-3 bg-white/[0.03] border border-white/[0.06] rounded-xl mb-3">
                <span className="text-xs text-slate-500 flex-1 truncate text-left">{shareUrl}</span>
              </div>
            )}
            <button
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-5">
        ...
        {/* ── Verdict banner ── */}
        <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-2xl border score-reveal ${rec.bg}`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className={`text-3xl font-black font-[family-name:var(--font-display)] ${rec.color}`}>{rec.label}</h1>
              <p className="text-xs text-slate-600 mt-1">This is an AI-generated suggestion, not a final decision.</p>
            </div>
          </div>
          <div className="flex items-center gap-3 flex-wrap badge-stagger">
            <div className="text-center px-5 py-3 bg-black/20 rounded-xl">
              <div className="text-2xl font-black text-white">{animatedScore}</div>
