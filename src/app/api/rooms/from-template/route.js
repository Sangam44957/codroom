...
export const POST = withAuthz(async (request) => {
  try {
    ...
    const { templateId, candidateName, title, pipelineId } = body;
    ...
    const room = await createNewRoom(
      {
        ...
        pipelineId: pipelineId || template.defaultPipelineId || null,
      },
      user.userId
    );
    ...
    console.error("[rooms/from-template] Error:", error);
