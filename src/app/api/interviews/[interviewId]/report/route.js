...
export const POST = withAuthz(async (request, { params }) => {
  ...
  const { background = true } = body;
  
  const result = await generateReport(interviewId, { background });
  if (result.error) return NextResponse.json({ error: result.error }, { status: result.status });
  
  if (result.queued) {
    audit({
      actorId: user.userId, actorEmail: user.email, actorRole: "interviewer",
      action: AuditActions.REPORT_GENERATED, resource: "report", resourceId: result.jobId,
      metadata: { interviewId, queued: true },
      request,
    });
    return NextResponse.json({ jobId: result.jobId, status: "generating" }, { status: 202 });
  }
