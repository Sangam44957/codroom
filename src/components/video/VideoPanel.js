...
export default function VideoPanel({ sharePeerId, onPeerIdReceived, emitCameraToggle, onRemoteCameraToggle, emitEmojiReaction, onEmojiReaction }) {
  ...
  async function initializeVideo() {
    ...
    try {
      ...
      try {
        ...
        console.warn("PeerJS module import failed (Turbopack), loading from CDN...");
        // Turbopack can't bundle PeerJS — load it from CDN as fallback
        Peer = await new Promise((resolve, reject) => {
          // Check if already loaded from a previous attempt
          if (window.peerjs?.Peer) {
            resolve(window.peerjs.Peer);
            return;
          }
          const script = document.createElement("script");
          script.src = "https://cdn.jsdelivr.net/npm/peerjs@1.5.5/dist/peerjs.min.js";
          script.onload = () => {
            if (window.peerjs?.Peer) resolve(window.peerjs.Peer);
            else if (window.Peer) resolve(window.Peer);
            else reject(new Error("PeerJS loaded but Peer class not found"));
          };
      ...
      if (!Peer) {
        console.error("PeerJS could not be loaded");
        setCallStatus("error");
        return;
      }
      ...
      setCallStatus(err.name === "NotAllowedError" ? "permission-denied" : "no-camera");
    }
  }
  ...
  return (
    <div className="flex flex-col gap-2">
            ...
            <div className="text-center">
              ...
              <p className="text-slate-500 text-xs">
                {remoteCameraOff
                  ...
                  : callStatus === "no-camera" ? "No camera found"
                  : callStatus === "permission-denied" ? "Camera permission denied"
      ...
      {callStatus === "permission-denied" && (
        <div className="px-3 py-2 bg-rose-500/10 border border-rose-500/20 rounded-lg">
          <p className="text-rose-400 text-xs leading-relaxed">
            <span className="font-semibold">Camera/mic access denied.</span>{" "}
            Allow access in your browser settings, then{" "}
            <button onClick={initializeVideo} className="underline hover:text-rose-300">retry</button>.
          </p>
        </div>
      )}
      ...
      <EmojiReactions onSendReaction={emitEmojiReaction} onRemoteReaction={onEmojiReaction} />
    </div>
  );
}