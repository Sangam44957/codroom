...
async function sendEmail({ to, subject, html }) {
  ...
  if (!sender.email) {
    logger.error("BREVO_SENDER_EMAIL is required when BREVO_API_KEY is set — skipping email send");
    return { skipped: true };
  }
  ...
  logger.info({ to, subject, from: sender.email }, "Sending email");
  ...
  if (!res.ok) {
    logger.error({ status: res.status, response: responseText }, "Brevo email error");
    throw new Error(`Brevo error ${res.status}: ${responseText}`);
  }

  logger.info({ response: responseText }, "Email sent successfully");
  ...
  try {
    ...
    logger.error({ error: parseError }, "Failed to parse Brevo response");
