...
export async function verifyTokenWithRevocation(token, userId) {
  ...
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { jwtIssuedAt: true },
    });
    
    if (!user) return null;
    
    const tokenIssuedAt = new Date(payload.iat * 1000);
    if (tokenIssuedAt < user.jwtIssuedAt) return null;
    
    return payload;
  } catch {
    return payload; // Fallback if DB unavailable
  }
}
...
export async function getCurrentUserWithDbCheck() {
  ...
  // Verify user still exists in DB and token not revoked
  try {
    const { default: prisma } = await import("@/lib/db");
    const user = await prisma.user.findUnique({
      where: { id: payload.userId },
      select: { id: true, jwtIssuedAt: true },
    });
    if (!user) {
      cookieStore.set(COOKIE_NAME, '', {
        ...SESSION_COOKIE_OPTIONS,
    ...
    if (payload.iat) {
      const tokenIssuedAt = new Date(payload.iat * 1000);
      if (tokenIssuedAt < user.jwtIssuedAt) {
        cookieStore.set(COOKIE_NAME, '', {
          ...SESSION_COOKIE_OPTIONS,
          maxAge: 0,
          expires: new Date(0)
