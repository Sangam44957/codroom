...
export async function generateReport(interviewId, options = {}) {
  const interview = await findInterviewById(interviewId);
  if (!interview) return { error: "Interview not found", status: 404 };
  ...
  if (background) {
    // Queue AI report generation as background job
    const reportGeneration = await prisma.$transaction(async (tx) => {
      ...
      if (current.report) return { existing: true, reportId: current.report.id };
      if (current.status === "generating") return { generating: true };
